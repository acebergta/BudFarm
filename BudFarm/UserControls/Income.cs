...
namespace BudFarm.UserControls
{
    public partial class Income : UserControl
    {
        ...
        private void Statistics()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString.ConnectionString))
            {
                try
                {
                    ...
                    //За месяц
                    command.CommandText = "SELECT SUM(inc_sum) FROM dbo.Income WHERE inc_date > getdate() - 31";
                    summa = command.ExecuteScalar();
                    label5.Text = summa.ToString() + " рублей";

                    //За квартал
                    command.CommandText = "SELECT SUM(inc_sum) FROM dbo.Income WHERE inc_date > getdate() - 91";
                    summa = command.ExecuteScalar();
                    label7.Text = summa.ToString() + " рублей";

                    //За полгода
                    command.CommandText = "SELECT SUM(inc_sum) FROM dbo.Income WHERE inc_date > getdate() - 182";
                    summa = command.ExecuteScalar();
        ...
        private void LoadData()
        {
            ...
            SqlCommand command = new SqlCommand();
            command = connection.CreateCommand();
            command.CommandText = "SELECT * FROM Income ORDER BY inc_id";
            ...
            if (textBox3.Text != "")
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString.ConnectionString))
                {
                    try
                    {
                        ...
                        SqlCommand command = new SqlCommand();
                        command = connection.CreateCommand();
                        command.CommandText = "DELETE FROM dbo.Income WHERE inc_id = @tranzId";
